...
- 平均を `75.0回転/k` 形式で表示する。平均は 1000 円あたりの回転（対象行の回転差分 ÷（投資単価 × 対象行数 ÷ 1000））。投資単価は基準玉数 × 貸出レート（既定 250 玉 × 4 円 = 1000 円）で、1000 円なら 1 打ち出しあたりの平均になる
...
- セッションに名前をつけて保存し、過去セッションを後から閲覧・削除できる。一覧では 1k 平均と総回転、投資額（貯玉含む円換算）・持ち玉（玉と換金円）・円換算差額（持ち玉−投資、+/-）を表示する
