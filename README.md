...
- 平均を `75.0回転/k` 形式で表示する。平均は 1000 円あたりの回転（対象の回転差分 ÷（円換算投資 ÷ 1000））。通常の打ち出しは 1 行 = 投資単価（基準玉数 × 貸出レート、既定 250 玉 × 4 円 = 1000 円）。遊技終了は使った持ち玉×貸出レート＋現金
...
- 遊技終了時は最終回転数・前回からの現金・終了時の残持ち玉をまとめて記録する（入力欄は折りたたみ、初期は閉じたまま）。使った玉は現在の持ち玉との差分。現金と持ち玉は同時に記録できる。回転差分は円換算投資ぶん平均に含める
